...
'大部分功能和train一样 主要是用训练后的模型做逆向减噪'
'主要区别是测试数据的batch可能和训练数据不一样 然后主要操作在sampling模块'
def generate(output_directory,
    ...
    for i, batch in enumerate(testing_data):    # [b, 100, 14]

        if masking == 'mnr':
            mask_T = get_mask_mnr(batch[0], missing_k)    # [100, 14]
            mask = mask_T.permute(1, 0)              # [14, 100]
            mask = mask.repeat(batch.size()[0], 1, 1)     # [b, 14, 100]
        ...
        batch = batch.permute(0,2,1)              # [b, 14, 100]
        
        start = torch.cuda.Event(enable_timing=True)
        end = torch.cuda.Event(enable_timing=True)
        start.record()

        sample_length = batch.size(2)        # 100
        sample_channels = batch.size(1)       # 14
        
        # 基本就是ddpm里的逆向采样 多了监督条件缺失掩码
        ...
        print('generated {} utterances of random_digit at iteration {} in {} seconds'.format(num_samples,  ckpt_iter, 
