...
#           batch_size_per_gpu,         # SA模型需要 S4和DiffWave不需要        
          learning_rate,
          use_model,
          only_generate_missing,       # 加噪方式 全局还是局部加噪
          masking,                # 缺失值产生方式
          missing_k):              # 可能是预测长度
    ...
    # 实验结果文件夹的名称 generate experiment (local) path 
    ...
    training_data = np.load(trainset_config['train_data_path'])         #  (8000, 100, 14)
    training_data = np.split(training_data, 160, 0)                #  160 (50, 100, 14) 
    training_data = np.array(training_data)                      #  (160, 50, 100, 14)  
    training_data = torch.from_numpy(training_data).float().cuda()       #  (160, 50, 100, 14)
    print('Data loaded')
    ...
    while n_iter < n_iters + 1:    # 有模型导入就不会运行
        
        for batch in training_data:        # [50, 100, 14]
